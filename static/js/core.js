...
function buildList(argument) {
    ...
    $.get("http://10.0.0.5/api/get_posts.php?mode=category&id=3&time=all", function(data){
        ...
        if(data) {
            var listAdapter = {
                ...
                "getView": function(position) {
                    ...
                    var preview_img_wrapper = $("<div>").addClass("preview_img_wrapper").append(
                        ...
                        ).append(img_lightbox_controls).appendTo(work_content_wrapper);
                    ...
                    $("<div>").addClass("work_author").append(
                        ...
                        $("<a>").attr("href", item.author.url).html(item.author.name)
                        ).appendTo(work_content_wrapper);
                    //build work_tags
                    ...
                    $("<div>").addClass("statistics_show_wrapper").append(
                        ...
                        $("<div>").addClass("downloads_icon")
                        ).append(
                        $("<span>").html(item.downloads)
                        )
                        ).append(
                        $("<div>").attr({
                            ...
                            $("<div>").addClass("comments_icon")
                        ).append(
                            $("<span>").html(item.comments)
                        )
                        ).append(
                        $("<div>").attr({
                            ...
                            $("<div>").addClass("favorites_icon")
                        ).append(
                            $("<span>").html(item.favorites)
                        )
                        ).appendTo(work_content_wrapper);
            ...
            gridList.find(".work_title a").ellipsis({"width":workTitleWidth, "useContainerPadding": true, "useContainerMargin": false});
        }
    },"json");
}